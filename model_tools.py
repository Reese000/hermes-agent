...
def get_tool_definitions(
    ...
    if quiet_mode:
        ...
        cache_key = (
            ...
            # Per-model tool descriptions (W2): without this a long-lived
            # process serving two model families (Gateway, delegated child
            # agents) would hand one family the other's cached descriptions.
            getattr(harness_profile, "name", None),
        )
...
def _compute_tool_definitions(
    ...
    # Per-model tool description overrides (W2).  ``None`` profile -> no
    # overrides -> byte-identical output to the pre-W2 behaviour.
    _overrides = getattr(harness_profile, "tool_description_overrides", None) or None
    _appends = getattr(harness_profile, "tool_description_appends", None) or None
    filtered_tools = registry.get_definitions(
        tools_to_include,
        quiet=quiet_mode,
        tool_description_overrides=_overrides,
        tool_description_appends=_appends,
    )
