...
class TestConfigAndWiring:
    ...
    def test_system_blocks_actually_calls_the_builder(self):
        ...
        Note: the wiring lives in ``system_prompt_parts`` — the cache-aware
        API — since the upstream refactor made ``system_blocks`` a thin
        compatibility shim over it. Guard the real wiring point.
        """
        from agent.coding_context import RuntimeMode

        src = inspect.getsource(RuntimeMode.system_prompt_parts)
        assert "build_repo_map_block" in src, (
            "RuntimeMode no longer builds the repo map - the "
