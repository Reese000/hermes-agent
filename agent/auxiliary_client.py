    ...
    try:
        ...
        try:
            ...
            if not _should_retry_same_provider(task, transient_err, ""):
                raise
            # Skip retries for continuous_work_critic to avoid timeout blowup
            # (retries multiply per-attempt timeout, exceeding hard cap)
            _max_transient_retries = 0 if task == "continuous_work_critic" else _transient_retry_count()
