...
def _run_conversation_turn(
    ...
    def _cw_enforce_before_exit(fr: str) -> bool:
        ...
        try:
            ...
            # Track consecutive critic timeouts — if the critic keeps failing,
            # fall back to the pattern-matching gate instead of blocking forever.
            if not hasattr(agent, "_cw_critic_timeout_count"):
                agent._cw_critic_timeout_count = 0
            _req = ""
            ...
            if nudge is None:
                ...
                return False
            # If critic returned empty critique (system error), count it.
            # After 3 consecutive empty critiques, fall back to pattern-matching.
            from agent.continuous_work_critic import parse_critic_response
            if not nudge.strip() or "Critic LLM call exceeded" in nudge:
                agent._cw_critic_timeout_count += 1
                if agent._cw_critic_timeout_count >= 3:
                    logger.warning("CW bypass-path: critic failed %d times, using fallback",
                                   agent._cw_critic_timeout_count)
                    from agent.continuous_work_gate import build_continuous_work_nudge
                    _work_tools = sum(1 for _m in s.messages[-40:]
                                      if isinstance(_m, dict) and _m.get("tool_calls"))
                    fallback = build_continuous_work_nudge(
                        final_response=fr, work_evidence_tools=_work_tools,
                        attempts=getattr(agent, "_continuous_work_nudges", 0))
                    if fallback is None:
                        return False  # fallback approves
                    nudge = fallback
            else:
                agent._cw_critic_timeout_count = 0
            try:
    ...
    while (s.api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        if _run_phase(begin_iteration, agent, s).action == "break":
            if _cw_enforce_before_exit(getattr(s, "final_response", "")):
                continue
            break
