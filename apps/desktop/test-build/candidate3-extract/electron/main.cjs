// Defensive: clear ELECTRON_RUN_AS_NODE if inherited from the parent shell.
// Some environments (Kilo Code agent sessions, CI runners, Electron
// development scripts) set ELECTRON_RUN_AS_NODE=1 in the process
// environment.  If that leaks into the packaged desktop app, the
// renamed Electron binary (Hermes.exe) interprets it as "run as Node
// instead of launching the GUI" and exits silently or prints Node
// help.  Deleting it here — before any Electron API is touched —
// ensures the app always starts as a real Electron window regardless
...
async function applyUpdates(opts = {}) {
  if (process.env.HERMES_DESKTOP_NO_AUTO_UPDATE === '1') {
    rememberLog('[updates] skipping apply (HERMES_DESKTOP_NO_AUTO_UPDATE=1)')
    return { ok: false, error: 'auto-update disabled', message: 'Auto-update is disabled (HERMES_DESKTOP_NO_AUTO_UPDATE=1). Use `hermes update` from the terminal to update manually.' }
  }
  if (updateInFlight) {
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  if (process.env.HERMES_DESKTOP_NO_AUTO_UPDATE === '1') {
    rememberLog(`[bootstrap] skipping ${reason} recovery handoff (HERMES_DESKTOP_NO_AUTO_UPDATE=1)`)
    return false
  }
  ...
  if (backend.kind === 'bootstrap-needed') {
    if (process.env.HERMES_DESKTOP_NO_AUTO_UPDATE === '1') {
      const devError = new Error(
        'Hermes backend not found but auto-update/bootstrap is disabled (HERMES_DESKTOP_NO_AUTO_UPDATE=1). ' +
        'Run `hermes update` or `hermes setup` from a terminal to repair the install, then relaunch.'
      )
      devError.isBootstrapFailure = true
      bootstrapFailure = devError
      throw devError
    }
