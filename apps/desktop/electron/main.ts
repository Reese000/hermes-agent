...
async function getRemoteUrl(updateRoot, remote) {
  const res = await runGit(['remote', 'get-url', remote], { cwd: updateRoot })
...
async function checkUpdates() {
  ...
  const originUrl = await getOriginUrl(updateRoot)
  // Fork installs: origin tracks the fork itself (always synced), so the
  // behind-count compares against upstream when configured — same decision
  // the backend banner check makes (banner._update_compare_ref).
  const upstreamUrl = await getRemoteUrl(updateRoot, 'upstream')
  const { remote: compareRemote, ref: compareRef } = resolveCompareTarget({
    branch,
    hasUpstream: Boolean(upstreamUrl)
  })
  const compareUrl = upstreamUrl || originUrl
  ...
  const fetched = await runGit(['fetch', '--quiet', compareRemote, branch], { cwd: updateRoot })
  ...
  const countStr = shouldCountCommits({ isShallow }) ? await git(['rev-list', `HEAD..${compareRef}`, '--count']) : ''
  ...
  const targetIsAncestorOfHead =
    ...
    (await runGit(['merge-base', '--is-ancestor', compareRef, 'HEAD'], { cwd: updateRoot })).code === 0
  ...
  if (behind === null) {
    behind = await fetchCompareBehindCount({ currentSha, originUrl: compareUrl, targetSha })
  }
  ...
  const commits = behind !== 0 ? await readCommitLog(updateRoot, branch, isShallow, compareRemote) : []
