...
class SelfHostedBackend(Mem0Backend):
    ...
    def __init__(self, api_key: str, host: str, transport=None, timeout: float = 30.0):
        ...
        # ``timeout`` covers full request time: the local-broker sync path (add with infer=True) runs 10-90 s server-side.
        self._client = httpx.Client(base_url=host.rstrip("/"), headers=headers, timeout=timeout, transport=transport or httpx.HTTPTransport(retries=2))
...
class OSSBackend(Mem0Backend):
    ...
    def close(self):
        # Close EVERY client individually. The old single-suppress chain aborted on
        # first failure, leaking mem0's secondary Qdrant clients (telemetry/migrations
        # store at ~/.mem0/migrations_qdrant, entity store) — a leaked client keeps its
        # EXCLUSIVE path lock, so this process could never reopen its own store.
        mem = self._memory
        self._memory = None
        if mem is None:
            return
        with suppress(Exception):
            telemetry = getattr(mem, "telemetry", None)
            if telemetry and hasattr(telemetry, "posthog"):
                with suppress(Exception):
                    telemetry.posthog.shutdown()
        # Read private attrs directly: mem0's entity_store property LAZILY CREATES
        # a store when accessed, which would defeat close() entirely.
        holders = [mem]
        for attr in ("vector_store", "_telemetry_vector_store", "_entity_store", "entity_store", "migrations_client"):
            with suppress(Exception):
                obj = mem.__dict__.get(attr)
                if obj is not None and obj not in holders:
                    holders.append(obj)
        for obj in holders:
            # Each close in its own suppress: one failure must not strand the rest.
            with suppress(Exception):
                client = obj.__dict__.get("client") if hasattr(obj, "__dict__") else None
